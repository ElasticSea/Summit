...
namespace Assets.Scripts.input{
    public class AccelerationHelper{
        private readonly IGameInput wrappedInput;

        /// <summary>
        /// Use this function for default class initialization.
        /// </summary>
        public AccelerationHelper(IGameInput wrappedInput){
        ...
        public void OnUpdate(){
            ...
            acceleration = CorrectRawAcceleration(lastAcceleration, wrappedInput.Direction);
        }
        ...
        /// <returns>Vector3</returns>
        public Vector3 LowPassFilter(Vector3 lastAcc, Vector3 currentAcc, float lowPassFilteringFactor){
        ...
        /// <returns>Vector3</returns>
        public Vector3 LowPassFilterAdaptive(Vector3 lastAcc, Vector3 currentAcc, float frequency,
            float cutoffFrequency, bool adaptive){
            var dt = 1.0f/frequency;
            var RC = 1.0f/cutoffFrequency;

            var filterConstant = dt/(dt + RC);

            var alpha = filterConstant;

            if (adaptive){
                var kAccelerometerMinStep = 0.02f;
                var kAccelerometerNoiseAttenuation = 3.0f;

                var d =
                    Mathf.Clamp(
                        Mathf.Abs(Norm(lastAcc.x, lastAcc.y, lastAcc.z) - Norm(currentAcc.x, currentAcc.y, currentAcc.z))/
        ...
        /// <returns>Vector3</returns>
        public Vector3 HighPassFilter(Vector3 lastAcc, Vector3 currentAcc, float highPassFilteringsFactor){
            // Subtract the low-pass value from the current value to get a simplified high-pass filter
            currentAcc.x = lastAcc.x -
                           ((lastAcc.x*highPassFilteringsFactor) + (currentAcc.x*(1.0f - highPassFilteringsFactor)));
            currentAcc.y = lastAcc.y -
                           ((lastAcc.y*highPassFilteringsFactor) + (currentAcc.y*(1.0f - highPassFilteringsFactor)));
            currentAcc.z = lastAcc.z -
                           ((lastAcc.z*highPassFilteringsFactor) + (currentAcc.z*(1.0f - highPassFilteringsFactor)));
        ...
        /// <param name="adaptive">true or false</param>
        /// <returns>Vector3</returns>
        public Vector3 HighPassFilterAdaptive(Vector3 lastAcc, Vector3 currentAcc, float frequency,
            float cutoffFrequency, bool adaptive){
            var dt = 1.0f/frequency;
            var RC = 1.0f/cutoffFrequency;
        ...
        /// </summary>
        private float accToleranceXAxis01 = 0.03f;

        private float accToleranceYAxis01 = 0.03f;
        private float accToleranceZAxis01 = 0.03f;

        private float accToleranceXAxis02 = 0.06f;
        private float accToleranceYAxis02 = 0.06f;
        private float accToleranceZAxis02 = 0.06f;
        ...
        /// <param name="lastAcc">Last Acceleration</param>
        /// <param name="currentAcc">Current Acceleration</param>
        private Vector3 CorrectRawAcceleration(Vector3 lastAcc, Vector3 currentAcc){
            var diffX = Mathf.Abs(currentAcc.x - lastAcc.x);
            var diffY = Mathf.Abs(currentAcc.y - lastAcc.y);
            var diffZ = Mathf.Abs(currentAcc.z - lastAcc.z);
            ...
            accToleranceXAxis02 = (diffX > accToleranceXAxis01 && diffX <= accToleranceXAxis02)
                ? diffX
                : accToleranceXAxis02;
            ...
            accToleranceYAxis02 = (diffY > accToleranceYAxis01 && diffY <= accToleranceYAxis02)
                ? diffY
                : accToleranceYAxis02;
            ...
            accToleranceZAxis02 = (diffZ > accToleranceZAxis01 && diffZ <= accToleranceZAxis02)
                ? diffZ
                : accToleranceZAxis02;

            //Correct the X axis if necessary.
            if (accToleranceXAxis01 == diffX){
                currentAcc.x = (currentAcc.x > 0)
                    ? currentAcc.x -= accToleranceXAxis01
                    : currentAcc.x += accToleranceXAxis01;
            }

            if (accToleranceXAxis02 == diffX){
                currentAcc.x = (currentAcc.x > 0)
                    ? currentAcc.x -= accToleranceXAxis02
                    : currentAcc.x += accToleranceXAxis02;
            }

            //Correct the Y axis if necessary.
            if (accToleranceYAxis01 == diffY){
                currentAcc.y = (currentAcc.y > 0)
                    ? currentAcc.y -= accToleranceYAxis01
                    : currentAcc.y += accToleranceYAxis01;
            }

            if (accToleranceYAxis02 == diffY){
                currentAcc.y = (currentAcc.y > 0)
                    ? currentAcc.y -= accToleranceYAxis02
                    : currentAcc.y += accToleranceYAxis02;
            }

            //Correct the Z axis if necessary.
            if (accToleranceZAxis01 == diffZ){
                currentAcc.z = (currentAcc.z > 0)
                    ? currentAcc.z -= accToleranceZAxis01
                    : currentAcc.z += accToleranceZAxis01;
            }

            if (accToleranceZAxis02 == diffZ){
                currentAcc.z = (currentAcc.z > 0)
                    ? currentAcc.z -= accToleranceZAxis02
                    : currentAcc.z += accToleranceZAxis02;
            }
